                ...
                <form action="{{ route('diskstorages.store') }}" method="post">
                    ...
                    <div class="mb-3 row">
                            <label for="status" class="col-md-4 col-form-label text-md-right text-left">Estado</label>
                                <div class="col-md-6">
                                    <div class="custom-control custom-switch mt-2"><input type="checkbox" class="custom-control-input @error('status') is-invalid @enderror" name="status" id='status' value="1" {{ old('featured') ? 'checked="checked"' : '' }}/><label class="custom-control-label" for="status">Activo</label></div>
